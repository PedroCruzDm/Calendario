    ...
    const handleSave = async () => {
        ...
        try {
            ...
            if (user && user.uid) {
                ...
                // Usuário logado? (sim) = salva na subcoleção do usuário
                const userEventsCollection = collection(db, "usuarios", user.uid, "eventos");
                docRef = await addDoc(userEventsCollection, newEvent);
                newEvent.id = docRef.id;
            } else {
                // Usuário logado? (não) = salva na coleção global
                const globalEventsCollection = collection(db, "eventos");
                docRef = await addDoc(globalEventsCollection, newEvent);
                newEvent.id = docRef.id;
            }
            ...
            console.warn("Erro ao salvar o evento. Verifique suas permissões.");
            toast.error("Erro ao salvar o evento. Verifique suas permissões.");
        }
    };
